...
/*std::vector<std::vector<unsigned>> make_batch_lookups(std::vector<unsigned> supported_batch_sizes) {
  std::vector<std::vector<unsigned>> lookups;
    std::vector<unsigned> lookup;
  lookup.push_back(0);
  for (auto batch_size : supported_batch_sizes) {
      auto prev = lookup[lookup.size()-1];
  ...
  for (unsigned i =0; i<2;i++) {
        copy(lookup.begin(), lookup.end(), back_inserter(lookups[i]));;
  }

  return lookups;
}*/

void lookupVal_(libconfig::Config &config, std::string key, std::vector<unsigned> lookup) {
...
std::vector<std::vector<unsigned>> make_batch_lookups(std::vector<unsigned> supported_batch_sizes) {
  ...
  */
  ...
  std::string base_filename = "/users/yiweiz/clockwork-modelzoo-volta-master/resnet50_v2/model";
  ...
  //try {
        std::string measurements_file = base_filename+".measurements"+std::to_string( i );
        ...
        for (auto batch_size : batch_sizes) {
            ...
            key << "b" << batch_size;
            lookupVal_(measurements, key.str(), lookup);
        }
