 ...
 public:
    ...
    class Model {
     ...
     private:
        ...
        std::vector<std::vector<unsigned>> batch_lookups_;
    ...
    public:
        ...
        uint64_t estimate(unsigned batch_size, unsigned gpu_id);

     private:

        // For num_requests requests, what is the maximum batch size we could execute?
        unsigned batch_lookup(unsigned num_requests, unsigned gpu_id = 0);
        unsigned index_lookup(unsigned batchsize);

        void check_timeouts(uint64_t free_at);
        uint64_t estimate(unsigned batch_size, int clock, unsigned gpu_id);
    };
