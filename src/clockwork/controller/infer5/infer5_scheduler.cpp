...
Scheduler::Model::Model(Scheduler* scheduler, BatchedModelState &state)
    ...
    //std::vector<unsigned> batch_lookup_ = util::make_batch_lookup(supported_batch_sizes);
    batch_lookups_ = util::make_batch_lookups(supported_batch_sizes);
    index_lookup_ = util::make_reverse_batch_lookup(supported_batch_sizes);
    max_batch_size = batch_lookups_[0].size() - 1;

    b1_exec = estimate(1,0); // Use this for slo_factor
}
...
std::vector<Scheduler::StrategyImpl> Scheduler::Model::new_strategies(int gpu_id, unsigned gpu_clock, int max_batchsize) {
    ...
    for (int i = queues.size()-1; i >= 0; i--) {
        ...
        strategy.priority = queue->front()->deadline - estimate(queue->batchsize, (unsigned) gpu_id);
...
void Scheduler::handle_request(Request &request) {
    ...
    request->demand = model->tracker->addRequest(
        model->estimate(1,0), request->exec_slo, request->weights_slo);
    model->invalidate_tracker();
